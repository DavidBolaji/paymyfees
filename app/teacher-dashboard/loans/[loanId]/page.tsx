...
import { DocumentsCardSkeleton } from '@/app/dashboard/loans/[loanId]/DocumentCardSkeleton';

export default function TeacherFullLoanInformationPage() {
  ...
  const calculateProgress = () => {
    if (!loanDetails?.installments) return { percentage: 0, paidCount: 0, totalCount: 0 };
    const paidCount = loanDetails.installments.filter(i => i.status === 'PAID').length;
    const totalCount = loanDetails.installments.length;
    return { percentage: totalCount > 0 ? Math.round((paidCount / totalCount) * 100) : 0, paidCount, totalCount };
  };

  const getNextRepayment = () => {
    if (!loanDetails?.installments) return { amount: 0, date: '-', installmentNumber: 0 };
    const next = loanDetails.installments.find(i => i.status === 'PENDING');
    if (next) {
      return {
        amount: next.amount,
        date: new Date(next.dueDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
  ...
  const getStatusValue = (status: string): 'ongoing' | 'completed' | 'pending' | 'cancelled' => {
    const map: Record<string, 'ongoing' | 'completed' | 'pending' | 'cancelled'> = {
      ACTIVE: 'ongoing', DISBURSED: 'ongoing', COMPLETED: 'completed',
      PENDING: 'pending', UNDER_REVIEW: 'pending', APPROVED: 'pending',
      REJECTED: 'cancelled', CANCELLED: 'cancelled', DEFAULTED: 'cancelled',
    };
    ...
    return (
      <div className="bg-[#F6F6F6] p-4 md:p-6 min-h-full">
        ...
        <div className="items-stretch gap-3 grid grid-cols-1 md:grid-cols-4">
          <div className="col-span-1 md:col-span-2"><InfoCardSkeleton itemCount={7} /></div>
          <div className="col-span-1 md:col-span-2"><InfoCardSkeleton itemCount={6} hasTopContent /></div>
        </div>
        <div className="items-stretch gap-3 grid grid-cols-1 md:grid-cols-4 my-6">
          <div className="col-span-1 md:col-span-2"><InfoCardSkeleton itemCount={5} /></div>
          <div className="col-span-1 md:col-span-2"><DocumentsCardSkeleton documentCount={3} /></div>
        </div>
      ...
      <div className="bg-[#F6F6F6] p-4 md:p-6 min-h-full">
        ...
        <div className="flex justify-center items-center h-[400px]">
          <div className="text-center">
            <p className="text-red-600 text-lg font-semibold mb-2">{error || 'Loan not found'}</p>
            <p className="text-[#7C7C7C]">Unable to load loan details. Please try again later.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-[#F6F6F6] min-h-full">
      <div className="pt-6 md:pt-0">
                ...
                topContent={
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-semibold text-[#7C7C7C] text-sm">Repayment Tracker</span>
                      <span className="font-semibold text-[#00296B] text-sm">{progress.percentage}% Completed</span>
                    </div>
          ...
          <div className="items-stretch gap-3 grid grid-cols-1 md:grid-cols-4 my-6">
                ...
                items={[
                  { label: 'Disbursed To', value: loanDetails.schoolName },
                  {
                    ...
                    value: loanDetails.disbursementDate
                      ? new Date(loanDetails.disbursementDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
                      : 'Pending',
                  },
            ...
            <div className="col-span-2 h-full">
              <div className="bg-white p-5 rounded-[16px] h-full">
                <h2 className="mb-5 font-semibold text-[#5F5F5F] text-[18px] leading-[1.2]">Documents Submitted</h2>
                    ...
                    {loanDetails.documents.map((document) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-6 h-6 text-[#00296B]">{getFileIcon()}</div>
                          <div>
                            <p className="font-semibold text-[#090909] text-[14px] leading-[1.2]">{document.fileName}</p>
                            <p className="font-bold text-[#7C7C7C] text-[11px] leading-[1.2]">{formatFileSize(document.fileSize)}</p>
                          </div>
                        </div>
                        <Download className="w-6 h-6 text-[#00296B] cursor-pointer" onClick={() => window.open(document.fileUrl, '_blank')} />
                      </div>
                    ))}
                  </div>
                ) : (
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
          <Button variant="navy-outline" className="flex-1 gap-2 h-[46px]" onClick={() => setIsContactSupportOpen(true)}>
            <Headphones className="w-6 h-6" />
            Contact Support
          </Button>
          <Button variant="navy-outline" className="flex-1 gap-2 h-[46px]" onClick={() => window.history.back()}>
            <X className="w-6 h-6" />
            Cancel
          </Button>
        </div>
