          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-stretch">
            ...
            <div className="bg-white rounded-xl px-5 pt-5 pb-4 h-full">
              ...
              {documents.length === 0 ? (
                ...
                const KYC_TYPES = ['NIN', 'SALARY_SLIP', 'BANK_STATEMENT', 'UTILITY_BILL', 'PARENT_PHOTO'];
                const kycDocs = documents.filter((d: any) => KYC_TYPES.includes(d.documentType));
                const loanDocs = documents.filter((d: any) => !KYC_TYPES.includes(d.documentType));

                const TYPE_LABELS: Record<string, string> = {
                  NIN: 'NIN Slip', SALARY_SLIP: 'Salary Slips', BANK_STATEMENT: 'Bank Statement',
                  UTILITY_BILL: 'Utility Bill', PARENT_PHOTO: 'Parent Photo',
                  STUDENT_PHOTO: 'Student Photo', SCHOOL_INVOICE: 'School Invoice / Offer Letter',
                  SCHOOL_RECEIPTS: 'School Receipts', PASSPORT: 'Passport Photo',
                };
                    ...
                    <div key={doc.id} className="flex items-center justify-between gap-3 bg-[#F6F6F6] rounded-lg px-3 py-2.5">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-[#7C7C7C]">{typeLabel}{doc.fileSize ? ` · ${fmtSize(doc.fileSize)}` : ''}</p>
                        </div>
                      </div>
                      <a
                        href={doc.fileUrl}
                ...
                return (
                  <div className="space-y-4">
                    {loanDocs.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-[#7C7C7C] uppercase tracking-wider mb-2">Loan Documents</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                          {loanDocs.map(renderDoc)}
                        </div>
                      </div>
                    )}
                    {kycDocs.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-[#7C7C7C] uppercase tracking-wider mb-2">KYC Documents</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                          {kycDocs.map(renderDoc)}
                        </div>
                      </div>
                    )}
                  </div>
